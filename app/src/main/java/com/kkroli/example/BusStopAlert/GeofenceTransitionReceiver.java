...
/*
 * This class handles the geofences, so that once they are entered, the user will receive a notification
 * as an alert.
 */
...
public class GeofenceTransitionReceiver extends WakefulBroadcastReceiver {
    ...
    // This method is triggered once a geofence is entered.
    @Override
    ...
    protected void onEnteredGeofences(String[] geofenceIds) {
        for (String fenceId : geofenceIds) {
            Toast.makeText(context, String.format("Approaching transit stop: %1$s", fenceId), Toast.LENGTH_SHORT).show();
            Log.i(TAG, String.format("Approaching transit stop: %1$s", fenceId));
